...
def show_speak_btn(lang: str) -> Button:
    stt_button = Button(label=st.session_state.locale.speak_btn, button_type="success", width=100)
    stt_button.js_on_event("button_click", CustomJS(code=get_js_code(lang)))
    return stt_button


def get_bokeh_result(lang: str) -> dict:
    stt_button = show_speak_btn(lang)
...
def show_voice_input(lang: str) -> None:
    if "input" not in st.session_state:
        st.session_state.input = {"text": "", "session": 0}
    result = get_bokeh_result(lang)
    if result:
